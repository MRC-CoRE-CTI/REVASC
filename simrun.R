...
sim_run <- function(nonnull, N_tot, nSims) {
...
### GLOBAL NULL ###
resgn <- sim_run(nonnull = rep(0, K), N_tot = 1500, nSims = nSims)

### GLOBAL ALTERNATIVE, VARY N ###
resga1 <- sim_run(nonnull = rep(1, K), N_tot = 1500, nSims = nSims)
resga2 <- sim_run(nonnull = rep(1, K), N_tot = 2000, nSims = nSims)
resga3 <- sim_run(nonnull = rep(1, K), N_tot = 2500, nSims = nSims)
resga4 <- sim_run(nonnull = rep(1, K), N_tot = 3000, nSims = nSims)

N_tot <- 3000
### PARTIAL ALTERNATIVE 1: ONLY SMALLEST subgroup (2) IS NULL
respa1 <- sim_run(nonnull = c(1, 0, 1, 1, 1, 1, 1), N_tot = N_tot, nSims = nSims)

### PARTIAL ALTERNATIVE 2: ONLY LARGEST subgroup (4) IS NULL
respa2 <- sim_run(nonnull = c(1, 1, 1, 0, 1, 1, 1), N_tot = N_tot, nSims = nSims)

### PARTIAL ALTERNATIVE 3: ONLY SMALLEST subgroup (2) IS NON-NULL
respa3 <- sim_run(nonnull = c(0, 1, 0, 0, 0, 0, 0), N_tot = N_tot, nSims = nSims)

### PARTIAL ALTERNATIVE 4: ONLY LARGEST subgroup (4) IS NON-NULL
respa4 <- sim_run(nonnull = c(0, 0, 0, 1, 0, 0, 0), N_tot = N_tot, nSims = nSims)

### PARTIAL ALTERNATIVE 5: THREE LARGEST subgroupS (5,1,4) ARE NULL
respa5 <- sim_run(nonnull = c(0, 1, 1, 0, 0, 1, 1), N_tot = N_tot, nSims = nSims)

### PARTIAL ALTERNATIVE 6: THREE LARGEST subgroupS (5,1,4) ARE NON-NULL
respa6 <- sim_run(nonnull = c(1, 0, 0, 1, 1, 0, 0), N_tot = N_tot, nSims = nSims)
